...
namespace WEB_API.Controllers
{
    ...
    public class UnitController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("[action]")]
        public async Task<IActionResult> AddUnit(string name, string code)
        {
            var result = await _Unit_Service.AddUnit(name, code);
        ...
        {
            var result = await _Unit_Service.UpdateUnit(unit.id, unit.name, unit.code);
            switch (result.success)
            {
                case true:
                    return Ok(result);
